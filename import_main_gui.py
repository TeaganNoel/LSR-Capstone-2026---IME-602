############################################################################
#Program Filename: import_main_gui.py
#Author: Mason Allen
...
def open_import_form():
    ImportForm_GUI.run(root, get_live_connection(), "lsr_testing_database")

def open_info_idnametool():
    IDNameKeyGenerator_GUI.run(root, get_live_connection(), "lsr_testing_database")

def open_import_csv():
    ImportCSV_GUI.run(root, get_live_connection(), "lsr_testing_database")
...
def open_runall():
    RunAll_GUI.run(root, get_live_connection(), "lsr_testing_database")
    return
...
ttk.Button(lf2, text="Format MaxxECU File", bootstyle=DARK).pack(fill='x', padx = x1, pady = y1) #NULL
